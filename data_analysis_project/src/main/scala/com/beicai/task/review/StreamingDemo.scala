package com.beicai.task.review
...
/**
  ...
  * spark-streaming 核心抽象 dstream
  * dstream:离散的数据输入流
  * spark-streaming编程步骤
  * 1，根据输入流（kafka,tcp,flume,hdfs,...），创建初始的dstream
  * 2，对初始的dstream进行不断的转换操作，形成一个又一个新的dstream
...
object StreamingDemo {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      * 的平稳运行
      */
    //sparkConf.set("spark.streaming.receiver.maxRate", "2000") //尽量不要用
    ...
    //1，根据输入流（kafka,tcp,flume,hdfs,...），创建初始的dstream
    //val lineDstream = ssc.socketTextStream("mini1", 8989, StorageLevel.MEMORY_AND_DISK)

    val lineDStreams = (0 until(3)).map(x => ssc.socketTextStream("hadoop-001",8989,StorageLevel.MEMORY_AND_DISK))
